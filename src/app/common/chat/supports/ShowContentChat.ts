    ...
    headerHtmlContentChat(reqDataContentChat) {
        ...
        if (reqDataContentChat.isTypeSingle) {
            if (reqDataContentChat.listParticipant.length) {
                reqDataContentChat.listParticipant.forEach((element) => {
                    htmlProfile += '<i channel="status.' + reqDataContentChat.dataChannelId + '" class="favorite-conversation ' + element.classStatus + '"'
                        + 'codePartId="' + element.codePartId + '" code-participant-id="' + element.users_id + '" aria-hidden="true"></i>';
                    htmlProfile += '<div class="info-group"><span class="status-name">' + element.moodMessageShow + '</span><span class="status-space">|</span><span class="status-other">fsgsgsgs</span></div>';
                });
            }
        } else {
        ...
        htmlProfile += '<div class="social-media">'
            ...
            + '<i id="list-contact-your" class="fa ' + (reqDataContentChat.isTypeSingle ? (reqDataContentChat.isFriendCurrentSingle ? "plus-group" : "") : "plus-user") + '"'
