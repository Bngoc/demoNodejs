...
PATHS=D:/Project/Demo/config/ssl
FILE_CA_KEY=$PATHS/$CA_KEY
FILE_CA_CERT=$PATHS/$CA_CERT

FILE_SERVER_KEY=$PATHS/$SERVER_KEY
FILE_SERVER_CSR=$PATHS/$SERVER_CSR

FILE_CLIENT_KEY=$PATHS/$CLIENT_KEY
FILE_CLIENT_CSR=$PATHS/$CLIENT_CSR
FILE_CLIENT_CERT=$PATHS/$CLIENT_CERT
...
#echo openssl req -new -key "$SERVER_KEY" -subj "C='$COUNTRY_NAME'/ST='$DEFINE_ST'/L='$DEFINE_L'/CN='$DEFINE_CN'/O='$ORG'" -out $SERVER_CSR
#echo openssl x509 -days $VALID_DAYS -req -in $SERVER_CSR -CAcreateserial -CA $CA_CERT -CAkey $CA_KEY -out $SERVER_CERT
echo "Done."

echo
echo "Creating Client certificate..."
#DEFINE_CN="Test user 1"
USER_ID="buingoc"
P12_PASSWORD=123456
#openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:$KEY_BITS -out $CLIENT_KEY
#echo openssl req -new -key $CLIENT_KEY -subj "/C='$COUNTRY_NAME'/ST='$DEFINE_ST'/L='$DEFINE_L'/OU='$DEFINE_OU'/O='$ORG'/CN='$DEFINE_CN'/emailAddress='$DEFINE_EMAIL'" -out $CLIENT_CSR
